        ...
        mongo.getAllRooms(patient.unit, function (rooms) {
          ...
          if(!roomFound){
            ...
            async function f1(){
              ...
              chosenBed = findBestNeighbour(patientList);
              addPatientToBed(chosenBed);
            }
...
async function getPatientByID(personNumber,patients){
  console.log("Getting patient by id");
  for(var i = 0; i < patients.length; i++){
    if(patients[i].personNumber == personNumber){
      return patients[i];
    }
  }
}
